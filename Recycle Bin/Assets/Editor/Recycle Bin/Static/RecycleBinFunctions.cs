...
{	
    ...
    {
        List<TrashFile> trash = new List<TrashFile>();

        string pathToRecycleBin = GetRecycleBin(false);

        if (Directory.Exists (pathToRecycleBin))
        {
            List<string> paths = new List<string>(Directory.GetFiles(pathToRecycleBin)); //Gets all paths
            paths.AddRange(Directory.GetDirectories(pathToRecycleBin)); //Gets all directories
